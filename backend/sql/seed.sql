INSERT INTO app_user (
    ...
    (4, 'admin', NULL, 'admin', '管理员', '管理员', '13600000000', NULL, NULL, NULL,
...
INSERT INTO task_order (
    id, task_id, order_code, title, address, salary, employer_id, employer_name,
    elderly_id, elderly_name, start_time, finish_time, status, settle_text
) VALUES
    (1, 1, '#2026040301', '社区医院陪同复查', '海棠社区卫生服务中心', 120, 2, '李先生',
     1, '王秀兰', '2026-04-03 14:00:00', NULL, 'working', '待结算'),
    (2, 3, '#2026040102', '上门陪伴老人聊天', '和睦家园 8 号楼', 80, NULL, '赵阿姨',
     1, '王秀兰', '2026-04-01 09:00:00', '2026-04-01 11:10:00', 'done', '模拟已结算')
ON CONFLICT (id) DO NOTHING;

SELECT setval('task_order_id_seq', GREATEST((SELECT COALESCE(MAX(id), 1) FROM task_order), 1), true);
